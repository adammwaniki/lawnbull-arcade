...
export default function ClientCardsMini({ image, title, subtitle, description, onViewMore }) {
  return (
    <div className="bg-[#fffff0] bg-opacity-80 p-6 rounded-lg shadow-lg max-w-sm mx-auto">
      <img src={image} alt={title} className="w-full h-48 object-cover rounded-md mb-4" />
      <h2 className="text-xl font-bold text-[#40240B] mb-2">{title}</h2>
      <h3 className="text-lg text-[#17163e] mb-2">{subtitle}</h3>
      <p className="text-[#40240B] mb-4">{description.slice(0, 32)}...</p>
      <button
        onClick={onViewMore}
